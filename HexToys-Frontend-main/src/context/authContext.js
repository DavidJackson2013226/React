...
export async function getUser(dispatch, address) {
    axios.get(`${process.env.REACT_APP_API}/user_info/${address}`)
        .then(async res => {
...
export async function loginUser(dispatch, account, nonce, signer) {
	try {
        ...
        try {
            ...
            if(signature) {
                const { data } = await axios.post(`${process.env.REACT_APP_API}/login`, Querystring.stringify({address: account, signature: signature}));
