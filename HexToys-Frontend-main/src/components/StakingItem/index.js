...
const StakingItem = (props) => {
  ...
  useEffect(() => {
    if (account && chainId && library && staking && (chainId === staking.chainId)) {
      getPendingRewards(staking.type, staking.address, account, chainId, library.getSigner())
        .then((value) => {
  ...
  }, [account, chainId, library, staking])
  ...
  const [clickedStakedNftInputIndex, setClickedStakedNftInputIndex] = useState(-1)
  ...
  const hanldeStakedNftInputOpenBtn = (index) => {
    ...
    setClickedStakedNftInputIndex(index);
  };
  ...
  async function putUnstake() {
    ...
    if (!(account && chainId && library && staking && (chainId === staking.chainId) && staking.stakeditems)) {
  ...
  const [clickedownedNftIndex, setClickedownedNftIndex] = useState(-1)
  ...
  async function putStake() {
    ...
    if (!(account && chainId && library && staking && (chainId === staking.chainId) && staking.owneditems)) {
    ...
    <>
      {
        ...
        <Element.Container>
          <div className="node-preview">
            <img className="node-img" src={staking.collectionInfo.image} alt="logo" />
          </div>
          <div className="node-main-content">
            <Element.ModalRow>
              <Element.ModalText1>Network</Element.ModalText1>
              <Element.ModalText3 onClick={() => {
                if (chainId && (chainId !== Number(staking.chainId))) {
                  setupNetwork(staking.chainId);
                }
              }}>
                {NetworkParams?.[staking.chainId]?.chainName}
              </Element.ModalText3>
            </Element.ModalRow>
            <Element.ModalRow>
            ...
            <Element.ModalRow>
              ...
              <Element.ModalText1>{formatNum(staking.stakeNftPrice)} {getCurrencyInfoFromAddress(staking.chainId, staking.rewardTokenAddress)?.symbol}</Element.ModalText1>
            </Element.ModalRow>
            <Element.ModalRow>
              <Element.ModalText1>Earn</Element.ModalText1>
              <Element.ModalText1>{getCurrencyInfoFromAddress(staking.chainId, staking.rewardTokenAddress)?.symbol}</Element.ModalText1>
            </Element.ModalRow>
            {
              account && chainId && (chainId === staking.chainId) &&
              <div>
                <Element.ModalRow>
...

            {
              ...
              <Element.EarnContainer>
                <div>
                  <Element.EarnTitle>
                    {getCurrencyInfoFromAddress(staking.chainId, staking.rewardTokenAddress)?.symbol} EARNED
                  </Element.EarnTitle>
                  <Element.EarnContent>
                    <Element.EarnValue>
            ...
            <Element.ModalRow>
              ...
              <Element.ModalText3 onClick={() => window.open(`/collection/${staking.chainId}/${staking.stakeNftAddress}`)}>
            ...
            </Element.TitleView>
            {
              ...
              <Element.DetailContent>
                ...
                <Element.BottomSubView>
                  <p onClick={() => window.open(`${NetworkParams?.[staking.chainId]?.blockExplorerUrls[0]}/address/${staking.address}`)}>
                    View Contract
                  </p>
                ...
                {
                  account && chainId && (chainId === staking.chainId) && (staking.rewardTokenAddress !== '0x0000000000000000000000000000000000000000') &&
                  <Element.BottomSubView
                    onClick={() => registerToken(staking.rewardTokenAddress, getCurrencyInfoFromAddress(staking.chainId, staking.rewardTokenAddress)?.symbol, getCurrencyInfoFromAddress(staking.chainId, staking.rewardTokenAddress)?.decimals)}>
                    <p>
                      Add to Metamask
                    </p>
      ...
      {
        account && chainId && staking && (chainId === staking.chainId) && stakedNFTModalOpen && staking.stakeditems &&
        <Modal
            ...
            <Element.NFTContainer>
              {
                staking.stakeditems.map((stakeditem, index) =>
                  <Element.NFTContent key={index}>
                    ...
                    >
                      <img src={stakeditem.itemInfo.image} alt="" />
                    </div>
                    {
                      stakedNFTOpen[index] &&
      ...
      {
        account && chainId && staking && (chainId === staking.chainId) && ownedNftModalOpen && staking.owneditems &&
        <Modal
